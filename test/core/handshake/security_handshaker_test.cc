...
class LoopbackMockEndpointController
    : public grpc_event_engine::experimental::BaseMockEndpointController {
 public:
  explicit LoopbackMockEndpointController(
  ...
  bool Read(absl::AnyInvocable<void(absl::Status)> on_read,
    ...
    if (read_buffer_.Count() > 0) {
      ...
      engine_->Run(
          [cb = std::move(on_read)]() mutable { cb(absl::OkStatus()); });
    } else {
  ...
  bool Write(absl::AnyInvocable<void(absl::Status)> on_writable,
    ...
    if (data->Count() > 0) {
      ...
      for (size_t i = 0; i < data->Count(); ++i) {
        ...
        written_data.append(reinterpret_cast<const char*>(slice.data()),
                            slice.size());
      }
      data->Clear();
      ...
      if (written_data.find("CLIENT_INIT") != std::string::npos) {
        ...
        MutexLock lock(&mu_);
        if (on_read_) {
          on_read_slice_buffer_->Append(
              grpc_event_engine::experimental::Slice::FromCopiedBuffer(
                  reinterpret_cast<const char*>(frame), 15));
          engine_->Run(
        ...
        if (on_read_) {
          on_read_slice_buffer_->Append(
              grpc_event_engine::experimental::Slice::FromCopiedBuffer(
                  reinterpret_cast<const char*>(frame), 19));
          engine_->Run(
              [cb = std::move(on_read_)]() mutable { cb(absl::OkStatus()); });
          on_read_ = nullptr;
          on_read_slice_buffer_ = nullptr;
 ...
 private:
  ...
  grpc_event_engine::experimental::SliceBuffer* on_read_slice_buffer_
...
TEST(SecurityHandshakerTest, MetricEmissionOnFailure) {
  ...
  handshaker->DoHandshake(
      args.get(), [&done, args, connector, handshaker](absl::Status status) {
        done.Notify();
      });
...
TEST(SecurityHandshakerTest, MetricEmissionOnSuccess) {
        ...
        if (instrument->name == "grpc.security.client.handshaker.duration") {
          if (!*hook_called) {
            *hook_called = true;
            ...
            // Target should be "dns:///localhost:50051" (from
            // args_->args.GetString(GRPC_ARG_SERVER_URI))
            EXPECT_EQ(labels[1], "dns:///localhost:50051");
            // Protocol should be "mock"
            EXPECT_EQ(labels[2], "mock");
            // Resumed should be "true" (because we set it to true in
            // check_peer)
  ...
  absl::Notification done;
  handshaker->DoHandshake(args.get(), [&done, loopback_ctrl, connector,
                                       handshaker, args](absl::Status status) {
